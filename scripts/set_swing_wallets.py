"""Manage the COPY swing-copy roster (wallet_pool.swing).

The swing (multi-day single-wallet follow-in) strategy fires on a buy from any
wallet flagged `swing = true`. The roster lives in the DB so it can be changed
WITHOUT a redeploy — run this script, then `docker compose restart bot_copy` for
the bot to pick it up immediately (it reloads the roster at startup).

IMPORTANT: a swing wallet only produces triggers if it is ALSO active tier — only
active-tier webhook events reach the copy:buys/copy:sells channels the bot
consumes. Use --ensure-active to promote flagged wallets (the seed roster includes
pruned wallets that are swing-followable-positive; they must be re-activated).

Usage (inside the framework container, or anywhere with DB access):
  python -m scripts.set_swing_wallets --list
  python -m scripts.set_swing_wallets --add ADDR1 ADDR2 --reason "seed 2026-09-07" --ensure-active
  python -m scripts.set_swing_wallets --add-file roster.txt --reason "seed" --ensure-active
  python -m scripts.set_swing_wallets --remove ADDR1
"""
    ...
    with open(path, "r", encoding="utf-8") as fh:
        for line in fh:
            ...
            if not line or line.startswith("#"):
                continue
            # tolerate "addr <anything>" or "addr,..." rows by taking the first field
            out.append(line.replace(",", " ").split()[0].strip())
    return out
...
def _print_roster() -> None:
    with session_scope() as s:
        rows = s.execute(text(
            "SELECT address, tier, swing_reason, swing_at, "
            "       cielo_pnl_90d_usd, pinned "
            "FROM wallet_pool WHERE swing = true "
            "ORDER BY tier, address"
        )).all()
    if not rows:
        print("swing roster is EMPTY")
        return
    print(f"swing roster ({len(rows)} wallets):")
    n_active = 0
    for r in rows:
        if r.tier == "active":
            n_active += 1
        ...
        print(f"  {r.address}  {pnl:>12}{pin}{flag}  {r.swing_reason or ''}")
...
def _apply(addresses: list[str], *, add: bool, reason: str, ensure_active: bool) -> int:
    ...
    with session_scope() as s:
        for addr in addresses:
            ...
            if add:
                s.execute(text(
                    "UPDATE wallet_pool SET swing = true, "
                    "swing_at = :now, swing_reason = :reason "
                    "WHERE address = :a"
                ...
                if row.tier != "active":
                    ...
                    if ensure_active:
                        s.execute(text(
                            ...
                            "promoted_at = :now, demoted_at = NULL WHERE address = :a"
                        ), {"now": _now(), "a": addr})
            else:
                s.execute(text(
                    "UPDATE wallet_pool SET swing = false WHERE address = :a"
                ), {"a": addr})
            changed += 1
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Manage the COPY swing roster.")
